import { Cartesian3 as cCartesian3, CallbackProperty } from 'cesium';
...
export class EditablePolyline extends AcEntity {
  ...
  private setMiddleVirtualPoint(firstP: EditPoint, secondP: EditPoint): EditPoint {
    ...
    const midPointCartesian3 = cCartesian3.lerp(new cCartesian3(pos1.x, pos1.y, pos1.z), new cCartesian3(pos2.x, pos2.y, pos2.z), 0.5, new cCartesian3());
  ...
  moveShape(startMovingPosition: Cartesian3, draggedToPosition: Cartesian3) {
    ...
    const delta = GeoUtilsService.getPositionsDelta(this.lastDraggedToPosition, new cCartesian3(draggedToPosition.x, draggedToPosition.y, draggedToPosition.z));
    this.positions.forEach(point => {
      const pos = point.getPosition()
      const newPos = GeoUtilsService.addDeltaToPosition(new cCartesian3(pos.x, pos.y, pos.z), delta, true);
