...
export class CesiumLongPressObserver extends CesiumPureEventObserver {
  ...
  constructor(protected event: CesiumEvent,
              protected modifier: CesiumEventModifier | undefined,
              private eventFactory: CesiumEventBuilder) {
    super(event, modifier);
  }

  init(): ConnectableObservable<any> {
    let startEvent: CesiumEvent | undefined;
    let stopEvent: CesiumEvent | undefined;
    ...
    if (startEvent === undefined || stopEvent === undefined) {
      throw new Error(`Unsupported event: ${this.event}`);
    }

    // Save start event position
    let startEventPosition: Cartesian2 | null = null;
